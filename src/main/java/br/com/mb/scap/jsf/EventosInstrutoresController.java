...
public class EventosInstrutoresController implements Serializable {
    ...
    @EJB
    private br.com.mb.scap.jpa.session.InstrutorFacade instrutorFacade;
    ...
    public void cadastrarTodos(){
        ArrayList<DataEvento> eventos = new ArrayList<>(dataEventoFacade.findAll());                   
        for (DataEvento evento : eventos) {
            EventosInstrutores eventosInstrutores = new EventosInstrutores();
            eventosInstrutores.setIdevento(evento.getIdevento());            
            eventosInstrutores.setIdinstrutor(current.getIdinstrutor());
            ejbFacade.create(eventosInstrutores);
            System.out.println("Evento: " + eventosInstrutores.getIdevento().getNome() + "Instrutor" + current.getIdinstrutor().getNome());
        }
    }
    ...
    public void removerTodos(){
        ...
        for (DataEvento evento : eventos) {
            ...
            eventosInstrutores.setIdevento(evento.getIdevento());            
            eventosInstrutores.setIdinstrutor(current.getIdinstrutor());
            ejbFacade.remove(eventosInstrutores);
            System.out.println("Evento: " + eventosInstrutores.getIdevento().getNome() + "Instrutor" + current.getIdinstrutor().getNome());
        }
    }
